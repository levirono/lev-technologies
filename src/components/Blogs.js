...
const Blogs = () => {
  ...
  const handleBlogClick = (index) => {
    ...
    // Scroll to the top when a blog is clicked
  ...
  return (
    <div>
      {expandedBlogIndex !== null && (
        <FullBlogView blog={blogs[expandedBlogIndex]} isExpanded={isBlogExpanded(expandedBlogIndex)} />
      )}
        ...
        <div className="container mx-auto">
          ...
          {blogs.map((blog, index) => (
            <div
              ...
              onClick={() => { handleBlogClick(index); rearrangeBlogs(index); }}
            >
              ...
              <div className="text-gray-700">
                {blog.content.map((item, itemIndex) => (
                  <React.Fragment key={itemIndex}>
                    {item.type === 'text' && (
                      <p className="mb-4 truncate max-w-[50]">
                        {item.text}
                      </p>
                    )}
                    {item.type === 'image' && (
